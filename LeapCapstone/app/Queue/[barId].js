...
const VenueProfile = () => {
  // useLocalSearchParams to get the dynamic parameter
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchVenue = async () => {
      try {
        // Fetch bar details using the barId from useLocalSearchParams
    ...
    fetchVenue();
  }, [barId, token]);  // Add barId as a dependency!
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.contentContainer}>
        ...
        {bar.imageUrl && (
          <Image
            source={{ uri: encodeURI(`http://localhost:5001${bar.imageUrl}`) }}
